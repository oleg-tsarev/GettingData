# === script preparation === #
setwd("~/Documents/GettingData/project")
rm(list=ls())


# === download and unzip data folder === #
#download.file("https://d396qusza40orc.cloudfront.net/getdata%2Fprojectfiles%2FUCI%20HAR%20Dataset.zip"
#              ,"~/Documents/GettingData/project/Dataset.zip",method = "wget")
#unzip(zipfile = "~/Documents/GettingData/project/Dataset.zip",overwrite = T,
#      exdir = "~/Documents/GettingData/project/")
...
left <- merge(left,activity_labels,by.x="activity_code",by.y="V1",all=T)
...
tidy <- aggregate(tidy,by=list(tidy$activity_label,tidy$subject),FUN=mean,na.rm=T)
tidy <- tidy[,!(colnames(tidy) %in% c("subject","activity_label"))]
names(tidy)[1:2] <- c("activity_label","subject")
